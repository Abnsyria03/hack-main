...
token = os.environ.get("5569133041:AAGZollpbOHEjDir-UT8_7_iAF0ggOYk_JY")
...
keyboard = [
  ...
  [
    Button.url("Owner", "https://t.me/Y66YY66")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ArabicHack = [
      [
        Button.url("اضغط هنا", f"https://t.me/TofedBOT?start=hack")
        ]
      ]         
    ...
    legendbye = [
      [
        Button.url("Must Join", f"https://t.me/Y66YY66")
        ]
      ]
...
async def op(event):
  ArabicHack = [
    [
      Button.url("اضغط هنا", f"https://t.me/Y66YY66")
      ]
    ]         
  await event.reply("Click Below To Use Me", buttons=ArabicHack)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond("SEND THIS ERROR TO - @Y66YY66\n**LOGS**\n" + str(e))
...
print("⚜️ Bot Deploy Successfully ⚜️ Kindly Join @Y66YY66|RAQAWI")
